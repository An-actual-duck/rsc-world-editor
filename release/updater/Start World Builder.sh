...
if [[ "${WORLD_BUILDER_SKIP_UPDATE:-0}" != 1 && -x "$ROOT_DIR/Update World Builder.sh" ]]; then
	if ! "$ROOT_DIR/Update World Builder.sh" --automatic; then
		printf 'WARNING: The automatic update check failed; continuing with the installed version.\n' >&2
	fi
fi
