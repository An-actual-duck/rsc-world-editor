...
ai_generate_workspace_guide() {
...

  {
    ...
    printf -- '- Branch: `%s`\n' "$branch"
    printf -- '- Checkpoint: `%s`\n\n' "$head"
    printf 'Project boundary: this worker belongs only to the RSC World Editor repository. Do not inspect or manage Spoiled Milk/Core-Framework branches, workers, releases, live-server state, or collaboration scripts. The ignored `.runtime-provider/` checkout is a frozen build dependency and is never a source worktree. Cross-repository runtime implementation and lock integration are coordinated by the product manager through the independent runtime manager.\n\n'
    if [[ "$phase" == IDLE ]]; then
      ...
      printf 'Work only on the assigned branch and task. Do not switch branches, use Git stashes, publish releases, or alter another worktree.\n\n'
