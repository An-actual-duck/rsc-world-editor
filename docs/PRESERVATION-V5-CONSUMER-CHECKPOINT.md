...
This is preparatory Editor work, not candidate acceptance. The worker still has
the previously published provider lock; manager adoption of the tested, published
v5 provider and genuine end-to-end conversion are pending.
...
The future-adoption branch requires the exact current versions 1–5 capability
and five-entry host probe matrix. Removing v5 is a refusal, not a fallback to v4.
The retired adaptive-v2 contract remains unchanged. This deliberate future
...
and matching reconciliation. Its v5 run awaits exact provider adoption.
